# -*- coding: utf-8 -*-
"""
Created on Sat Dec  3 16:11:08 2022
...
def plot(CSVNAMES,ind_to_be_plotted):
    ...
    for i,CSVNAME in enumerate(CSVNAMES):
        ...
        if i == ind_to_be_plotted:
            ...
            ax.plot(X,Y,marker=markers[i], color = colorlist[i],linestyle='None',label=r'Messwerte für $c='+str(int(cs[i]))+'\%$')
    ...
    the_table = the_table = plt.table(cellText=cell_text,
                    #rowLabels = np.full(7,'a'),
                      ...
                      colLabels=[r'$c='+str(i)+'\% $' for i in cs],
                      loc='bottom',
                      cellLoc='center',
                      bbox=[.1,-0.31,.8,0.2],
                      edges='closed')
    ...
    #----Save Figure-----------
    plt.savefig('Viscosity_by_Concentration_Sucrose_gamma_'+str(gammadotList[ind_to_be_plottet])+'s-1.pdf',dpi=1200)
        
if __name__ == "__main__":
    for i in range(0,11):
        plot(['sac_00_percent','sac_10_percent','sac_20_percent','sac_40_percent'],i)